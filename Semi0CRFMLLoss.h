					...
					if (idx == 0) {
						...
						for (int j = 0; j < labelSize; ++j) {
							for (int prevdist = 1; prevdist <= idx && prevdist <= maxLens[j]; prevdist++) {
								buffer.push_back(x[idx][dist]->val[i] + alpha[idx - prevdist][prevdist - 1][j]);
							}
						}
						...
						for (int j = 0; j < labelSize; ++j) {
							for (int prevdist = 1; prevdist <= idx && prevdist <= maxLens[j]; prevdist++) {
								buffer.push_back(x[idx][dist]->val[i] + alpha_answer[idx - prevdist][prevdist - 1][j]);
							}
						}
					...
					if (idx == seq_size) {
						...
						for (int j = 0; j < labelSize; ++j) {
							for (int nextdist = 0; nextdist < seq_size - idx && nextdist < maxLens[j]; nextdist++) {
								buffer.push_back(x[idx][nextdist]->val[j] + belta[idx][nextdist][j]);
							}
						}
						...
						for (int j = 0; j < labelSize; ++j) {
							for (int nextdist = 0; nextdist < seq_size - idx && nextdist < maxLens[j]; nextdist++) {
								buffer.push_back(x[idx][nextdist]->val[j] + belta_answer[idx][nextdist][j]);
							}
						}
			...
			for (int i = 0; i < labelSize; ++i) {
				for (int dist = 0; dist < seq_size - idx && dist < maxLens[i]; dist++) {
					...
					if (idx == 0) {
						...
						maxScores[idx][dist][i] = x[idx][dist]->val[i] + maxscore;
