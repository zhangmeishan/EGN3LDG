...
void to_vector(const vector<Tensor1D *>& vec_tensor, vector<vector<dtype>* >& vec_scores) {
	int size = vec_tensor.size();
	assert(size == vec_scores.size());
	for (int idx = 0; idx < size; idx++) {
		const Tensor1D *tensor = vec_tensor[idx];
		vector<dtype> *score = vec_scores[idx];

		int dim = tensor->dim;
		score->resize(dim);
		for (int idy = 0; idy < dim; idy++) {
			(*score)[idy] = tensor->v[idy];
		}
	}
